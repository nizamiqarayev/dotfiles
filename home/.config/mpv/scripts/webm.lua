...
expand_properties = function(text, magic)
    ...
    for prefix, raw, prop, colon, fallback, closing in text:gmatch("%" .. magic ..
        ...
        if colon == ":" then
            local _
            text, _ = text:gsub("%" .. magic .. "{" .. prefix .. raw .. original_prop:gsub("%W", "%%%1") .. ":" ..
                                    fallback:gsub("%W", "%%%1") .. closing .. "}", expand_properties(prop_value))
        else
            local _
...
monitor_dimensions = function()
    local properties = {"keepaspect", "video-out-params", "video-unscaled", "panscan", "video-zoom", "video-align-x",
                        "video-pan-x", "video-align-y", "video-pan-y", "osd-width", "osd-height"}
...
read_logfile_into_stats_array = function(logfile_path)
    ...
    for offset = 1, #logfile_string, stats_size do
        local bytes = {logfile_string:byte(offset, offset + stats_size - 1)}
...
do
    ...
    local _base_0 = {
        ...
        getPostFilters = function(self)
            return {"format=yuv444p16", "lavfi-scale=in_color_matrix=" .. self:getColorspace(), "format=bgr24"}
        end
    }
    _base_0.__index = _base_0
    ...
    local _base_0 = {
        getPreFilters = function(self)
            ...
            if colormatrixFilter[colormatrix] then
                append(ret, {"lavfi-colormatrix=" .. tostring(colormatrixFilter[colormatrix]) .. ":bt601"})
            end
            return ret
        end,
        getFlags = function(self)
            return {"--ovcopts-add=threads=" .. tostring(options.libvpx_threads)}
        end
    }
    _base_0.__index = _base_0
    setmetatable(_base_0, _parent_0.__base)
...
do
    ...
    local _base_0 = {
        ...
        observe_properties = function(self)
            ...
            local properties = {"keepaspect", "video-out-params", "video-unscaled", "panscan", "video-zoom",
...
append_track = function(out, track)
    ...
    if track['external'] and string.len(track['external-filename']) <= 2048 then
        ...
        return append(out, {"--" .. tostring(internal_flag[track['type']]) .. "=" .. tostring(track['id'])})
    end
end
...
append_audio_tracks = function(out, tracks)
    ...
    if #internal_tracks > 1 then
        ...
        return append(out, {"--lavfi-complex=" .. tostring(filter_string)})
    else
        if #internal_tracks == 1 then
...
get_scale_filters = function()
    if options.scale_height > 0 then
        return {"lavfi-scale=-2:" .. tostring(options.scale_height)}
    end
    return {}
end
...
get_fps_filters = function()
    if options.fps > 0 then
        return {"fps=" .. tostring(options.fps)}
    end
    return {}
end
...
append_property = function(out, property_name, option_name)
    ...
    if prop and prop ~= "" then
        return append(out, {"--" .. tostring(option_name) .. "=" .. tostring(prop)})
    end
end
...
append_list_options = function(out, property_name, option_prefix)
    ...
    if prop then
        for _index_0 = 1, #prop do
            ...
            append(out, {"--" .. tostring(option_prefix) .. "-append=" .. tostring(value)})
        end
    end
end
...
get_speed_flags = function()
    ...
    if speed ~= 1 then
        append(ret, {"--vf-add=setpts=PTS/" .. tostring(speed), "--af-add=atempo=" .. tostring(speed),
...
get_metadata_flags = function()
    ...
    return {"--oset-metadata=title=%" .. tostring(string.len(title)) .. "%" .. tostring(title)}
end
...
get_video_filters = function(format, region)
    ...
    if region and region:is_valid() then
        append(filters,
            {"lavfi-crop=" .. tostring(region.w) .. ":" .. tostring(region.h) .. ":" .. tostring(region.x) .. ":" ..
                tostring(region.y)})
    end
...
get_video_encode_flags = function(format, region)
    ...
    for _index_0 = 1, #filters do
        ...
        append(flags, {"--vf-add=" .. tostring(f)})
    end
...
find_path = function(startTime, endTime)
    ...
    if is_stream then
        if mp.get_property('file-format') == 'hls' then
            ...
            mp.command_native({'dump_cache', seconds_to_time_string(startTime, false, true),
                               seconds_to_time_string(endTime + 5, false, true), path})
...
encode = function(region, startTime, endTime)
    ...
    local command = {"mpv", path, "--start=" .. seconds_to_time_string(startTime, false, true),
                     "--end=" .. seconds_to_time_string(endTime, false, true), "--loop-file=no", "--no-pause"}
    ...
    if format.acceptsBitrate then
        if options.target_filesize > 0 then
            ...
            if video_bitrate then
                append(command, {"--ovcopts-add=b=" .. tostring(video_bitrate) .. "k"})
            end
            if audio_bitrate then
                append(command, {"--oacopts-add=b=" .. tostring(audio_bitrate) .. "k"})
            end
            if options.strict_filesize_constraint then
                local type = format.videoCodec ~= "" and "ovc" or "oac"
                append(command, {"--" .. tostring(type) .. "opts-add=minrate=" .. tostring(bitrate) .. "k",
                                 "--" .. tostring(type) .. "opts-add=maxrate=" .. tostring(bitrate) .. "k"})
            end
        else
            local type = format.videoCodec ~= "" and "ovc" or "oac"
            append(command, {"--" .. tostring(type) .. "opts-add=b=0"})
        end
    end
    ...
    if not options.strict_filesize_constraint then
        ...
        if options.crf >= 0 then
            append(command, {"--ovcopts-add=crf=" .. tostring(options.crf)})
        end
    end
    local dir = ""
    if is_stream then
    ...
    append(command, {"--o=" .. tostring(out_path)})
    if options.twopass and format.supportsTwopass and not is_stream then
        ...
        append(first_pass_cmdline, {"--ovcopts-add=flags=+pass1"})
        ...
        append(command, {"--ovcopts-add=flags=+pass2"})
...
do
    ...
    local _base_0 = {
        ...
        draw = function(self)
            ...
            ass:append(tostring(bold('1:')) .. " change point A (" .. tostring(self.pointA.x) .. ", " ..
                           tostring(self.pointA.y) .. ")\\N")
            ass:append(tostring(bold('2:')) .. " change point B (" .. tostring(self.pointB.x) .. ", " ..
                           tostring(self.pointB.y) .. ")\\N")
            ...
            ass:append(tostring(bold('ENTER:')) .. " confirm crop (" .. tostring(width) .. "x" .. tostring(height) ..
                           ")\\N")
    ...
    local _base_0 = {
        ...
        setValue = function(self, value)
            ...
            elseif "list" == _exp_0 then
                ...
                if not set then
                    return msg.warn("Tried to set invalid value " .. tostring(value) .. " to " ..
                                        tostring(self.displayText) .. " option.")
                end
            end
        end,
...
do
    ...
    _class_0 = setmetatable({
        __init = function(self, callback)
            ...
            local formatIds = {"webm-vp8", "webm-vp9", "mp4", "mp4-nvenc", "raw", "mp3", "gif"}
            local formatOpts = {
                possibleValues = (function()
                    ...
                    for _index_0 = 1, #formatIds do
                        ...
                        _accum_0[_len_0] = {fId, formats[fId].displayName}
                        _len_0 = _len_0 + 1
                    end
                    return _accum_0
                end)()
            }
            self.options = {{"output_format", Option("list", "Output Format", options.output_format, formatOpts)},
...
do
    ...
    local _base_0 = {
        ...
        draw = function(self)
            ...
            ass:append(tostring(bold('1:')) .. " set start time (current is " ..
                           tostring(seconds_to_time_string(self.startTime)) .. ")\\N")
            ass:append(tostring(bold('2:')) .. " set end time (current is " ..
